...
public sealed class ShippoOpenApiClientUtil : IShippoOpenApiClientUtil
{
    ...
    public ShippoOpenApiClientUtil(IShippoOpenApiHttpClient httpClientUtil, IConfiguration configuration)
    {
        _client = new AsyncSingleton<ShippoOpenApiClient>(async token =>
        {
            HttpClient httpClient = await httpClientUtil.Get(token).NoSync();

            var apiKey = configuration.GetValueStrict<string>("Shippo:ApiKey");
            string authHeaderValueTemplate = configuration["Shippo:AuthHeaderValueTemplate"] ?? "ShippoToken {token}";
